...
*/
      $("#calc").bind('click', function () {
          ...
          get_htmldata('view/cfkp_product/undefined', {
              'filter': f
          }, $("#finding_products"));
  ...
  function get_filter(p) {
      ...
      */
      ...
      if (p.goal == 'Капитальные вложения и пополнение оборотных средств') {
          goal1 = 'Пополнение оборотных средств';
          goal2 = 'Капитальные вложения';
      } else {
          goal1 = p.goal;
          goal2 = p.goal;
      };
      ...
      var f = {
          'and': [
              {
                  'and': [{
                      'data.program_criteria.min_sum': {
                          'lte': fin_amont
                      }
                  }, {
                      'data.program_criteria.max_sum': {
                          'gte': fin_amont
                      }
                  }]
              }
       , {
                  'and': [{
                      'data.program.max_month_limit': {
                          'gte': Number(p.fin_period)
                      }
                  }]
              }
       , {
