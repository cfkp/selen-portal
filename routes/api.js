...
router.post('/loadclass/:meta_class/:parent_name/:parent_id', checkAuth, function (req, res, next) {
    ...
    var child_coll = {
        'meta_parent_field': parent_name,
        'meta_parent_value': parent_id
    };
...
router.post('/load_main_menu', checkAuth, function (req, res, next) {
    ...
    if (req.user.role) {
        search_filter = {
            "_id": req.user.role.data.user_menu
        };
    } else {
        search_filter = {
            "name": "person_menu"
			...
			function (obj, callback) {
                ...
                if (new_state_act === 'Выполнено') {
                    if (obj.state === 'В работе') {
                        new_state = "На проверку"
                    } else if (obj.state === 'На проверку') {
                        new_state = "На рассмотрение"
                    } else if (obj.state === 'На рассмотрение') {
                        new_state = "Одобрено КМСП"
                    };
                    ...
                    if (obj.state === 'В работе') {} else if (obj.state === 'На проверку') {
                        new_state = "В работе"
                    } else if (obj.state === 'На рассмотрение') {
                        new_state = "Отказано КМСП"
                    };
		...
		function (res, callback) {
                ...
                if (res.user.state = 'new' && institute && institute === 'Корпорация МСП') {
                    ...
                    callback(null, {
                        'msg': {
                            'type': 'msg',
                            'msg': 'Ваша заявка зарегистрирована, для подтверждения Вам выслано письмо.'
                        }
                    })
                    ...
                    callback(null, {
                        'msg': {
                            'type': 'msg',
                            'msg': 'Ваша заявка зарегистрирована в вашем Личном кабинете, для работы с ней зайдите в личный кабинет'
                        }
                    })
                    ...
                    callback({
                        'msg': {
                            'type': 'error',
                            'msg': 'Указана програма c неизвестным институтом' + institute
                        }
                    })
