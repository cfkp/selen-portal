...
router.post('/ref_value_list', function (req, res, next) {
    ...
    async.waterfall([
			 function (callback) {
            var rows = db.get().collection(meta_class).find(req_filter, selcols).limit(5).toArray(function (err, rows) {
                var result = {};
                result = rows;
...
router.post('/audit/vw_collection_audit', function (req, res, next) {
    ...
    var meta_view = 'vw_collection_audit';
    var user_filter;
    var collection;
    if (req.body) {
        user_filter = req.body.filter;
    ...
    async.waterfall([
            ...
            if (!model) {
                res.status(500).json({
                    ...
                    'msg': 'Не найдено представление' + meta_view
                });
                return;
